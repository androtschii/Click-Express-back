...
namespace ClickExpress.BusinessLogic.Core.News
{
    public class NewsActions
    {
        protected List<NewsArticleDTO> ExecuteGetAllNewsAction(bool onlyPublished)
        {
            ...
            {
                var query = db.NewsArticles.Include(n => n.Author).AsQueryable();
                if (onlyPublished) query = query.Where(n => n.IsPublished);

                return query.OrderByDescending(n => n.PublishedAt).Select(n => new NewsArticleDTO
                {
                    Id = n.Id, Title = n.Title, Content = n.Content, ImageUrl = n.ImageUrl,
                    AuthorId = n.AuthorId, AuthorName = n.Author.Username,
        ...
        protected NewsArticleDTO? ExecuteGetNewsByIdAction(int id)
        {
            using (var db = new OrderContext())
            {
                var n = db.NewsArticles.Include(n => n.Author).FirstOrDefault(n => n.Id == id);
        ...
        protected ResponseAction ExecuteCreateNewsAction(int authorId, CreateNewsDTO dto)
        {
            using (var db = new OrderContext())
            {
                var article = new NewsArticleData
                {
                    ...
                    AuthorId = authorId, IsPublished = dto.IsPublished, PublishedAt = DateTime.Now
                };
        ...
        protected ResponseMsg ExecuteUpdateNewsAction(int id, CreateNewsDTO dto)
        {
            using (var db = new OrderContext())
            {
                ...
                article.Title = dto.Title; article.Content = dto.Content;
                article.ImageUrl = dto.ImageUrl; article.IsPublished = dto.IsPublished;
                db.SaveChanges();
